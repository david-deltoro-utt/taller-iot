...
class AdafruitMQTT():     
    ...
    ADAFRUIT_IO_KEY = ''
    ADAFRUIT_IO_USERNAME = 'davidtoro'
    client = MQTTClient(ADAFRUIT_IO_USERNAME, ADAFRUIT_IO_KEY)        
    
    def __init__(self, feeds=[]):
        self.name_feeds=feeds
        
    #def connect_component(self,client):
    def connect_component(self):
        self.client.name_feeds=self.name_feeds
    ...
    def run(self):
        self.connect_component(self)
        self.client.loop_background()
        while True:
            #value = random.randint(0, 100)
            #print('Publishing {0} to foo.'.format(value))
            #client.publish('foo', value)
            time.sleep(10)        
    ...
    def connected(self,client):
            ...
            for f in client.name_feeds:
                print('Connecting to {0}'.format(f))
                client.subscribe(f)
...
ADAFRUIT_IO_KEY = 'aio_LaIb20FSOjsa74jKVnPVHa1USh3Y'
